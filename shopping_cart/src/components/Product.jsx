...
const Product = ({post}) => {
    ...
    const  {cart} = useSelector((state)=>state);//cart is a empty array 
  ...
  return (
    <div className='flex flex-col items-center justify-between shadow-md hover:scale-110 transition duration-300 ease-in gap-3 p-4 mt-10 ml-5 rounded-xl hover:shadow-[rgba(0,_0,_0,_0.4)_0px_30px_90px]'>
        ...
        <div className='flex justify-between gap-12 items-center w-full mt-5'>
         ...
         {
            cart.some((p)=>p.id== post.id) ? //Agar cart ke andr post padi hai toh remove wala button dikhao otherwise  '
